    ...
    try {
        ...
        if (expandedKeys.value.length === 0) {
            ...
            if (selectedKeys.value.length === 0) {
                ...
                if (item) {
                    // 默认选中第一个
                    setSelectedKey(item.orgid, item)
                }
            } else {
        ...
        if (result.length == 0) {
            ...
            if (expandedKeys.value.length > 0) {
                ...
                for (let key of expandedKeys.value) {
                    if (result.findIndex((item) => item.orgid === key) !== -1) {
                        subKeys.push(key)
                    }
                }
                if (subKeys.length > 0) {
...
function onAddChildDepart(data = currentDepart.value) {
    ...
    const record = { parentid: data.orgid }
...
async function onDelete(data) {
    if (data) {
        ...
        doDeleteDepart([data.orgid], false)
    }
}
