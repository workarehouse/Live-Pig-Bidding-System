<template>
  <div>
        ...
        <a-row :gutter="24">
          ...
          <a-col :lg="8">
            <a-form-item label="日期">
              <a-date-picker value-format="YYYY-MM-DD" placeholder="开始日期" v-model:value="queryParam.credat_begin"
                class="query-group-cust" :allowClear="true" />
              <span class="query-group-split-cust">~</span>
              <a-date-picker value-format="YYYY-MM-DD" placeholder="结束日期" v-model:value="queryParam.credat_end"
                class="query-group-cust" :allowClear="true" />
            </a-form-item>
          </a-col>
          <a-col :lg="8">
            <a-form-item label="关键字">
              <a-input placeholder="请输入关键字" v-model:value="queryParam.keyword" :allowClear="true"></a-input>
            </a-form-item>
          </a-col>
    ...
    <!-- 新增 -->
    <a-modal :width="800" v-model:visible="visibleAdd" centered title="新增" @ok="handleAddClose" @cancel="onResetAdd">
      <a-card>
          ...
          <a-row>
            ...
            <a-col :span="12">
              <a-form-item label="交割日期">
                <a-date-picker :disabled-date="disabledDate" v-model:value="formAdd.saldat" style="width: 100%" />
              </a-form-item>
            </a-col>

            <a-col :span="12">
              <a-form-item label="猪场">
                <a-select v-model:value="formAdd.farmcod" show-search option-filter-prop="label" :options="farmerList"
                  @change="onFarmChange" :allowClear="true" placeholder="请选择猪场" style="width: 100%" />
              </a-form-item>
            </a-col>
            <a-col :span="12">
              <a-form-item label="猪品种">
                <j-dict-select-tag v-model:value="formAdd.pigType" dictCode="pig_type" placeholder="请选择猪品种"
            ...
            <a-col :span="24">
              <a-form-item :labelCol="{ sm: { span: 3 } }" label="客户名称" name="custcod">
                <a-select :filter-option="filterOption" v-model:value="formAdd.custcod" show-search placeholder="请选择客户"
                  :options="dictOptions">
                </a-select>
              </a-form-item>
            </a-col>
            <a-col :span="24">
              <a-form-item :labelCol="{ sm: { span: 3 } }" label="关联原订单">
                ...
                <a-select show-search :filterOption="filterOptionOrder" ref="select" v-model:value="formAdd.oldordno"
                  placeholder="请选择要关联的原订单">
            ...
            <a-col :span="24">
              <a-form-item :labelCol="{ sm: { span: 3 } }" label="客户需求">
                <a-textarea v-model:value="formAdd.custreq" :rows="2" placeholder="请输入客户需求"></a-textarea>
              </a-form-item>
            </a-col>
...
onMounted(() => {
  ...
  defHttp.post({ url: '/szjj/baseFarmer/getFarmerByLoginAcct', params: {} }).then((res) => {
    farmerList.value = (res || []).map((item) => ({ label: item.farmnam, value: item.farmcod }))
  })
...
// 使用 options 数组时，Antd 默认的搜索方式会匹配 label，因此不需要自定义 filter 函数
