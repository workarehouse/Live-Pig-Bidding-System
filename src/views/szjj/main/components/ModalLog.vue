<template>
    <BasicModal
        ...
        :centered="false"
    >
        <Description title="猪源信息" :collapseOptions="{ canExpand: false }" :column="2" :data="mockData2" :schema="schema" :width="850" />
...
const logTableSchema: BasicColumn[] = [
    {
        ...
        align: 'left',
        customRender: ({ record }) => {
            const log = `${record.credat} 客户${record.orderStatus == 2 ? record.creusrnam : '***'} 出价${record.price}元/公斤 ${record.pigcnt}头`
            // state 为「黑」时表示该出价异常
...
export default defineComponent({
    ...
    setup() {
        ...
        async function initData(data) {
            ...
            logTableData.value = resp_data.bidlog.map((item) => {
                return {
                    ...item,
                    orderStatus: resp_data.state
                }
            })
        }
