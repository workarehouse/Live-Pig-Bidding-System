...
export const columns: BasicColumn[] = [
    ...
    },
    {
        ...
        customRender: ({ text, record, index }) => {
            if (record['endDatetime'] == null) return ''
            else return !text ? '' + '~' + record['endDatetime'] : text + '~' + record['endDatetime'].slice(-5)
        }
    },
    {
        title: '竞拍结束时间',
...
export const formSchema: FormSchema[] = [
    ...
    },
    {
        ...
        componentProps: ({ formModel }) => {
            return {
                precision: 0,
                onChange: (e) => {
                    if (e && formModel.maxwgt) {
                        formModel.avgwgt = ((e + formModel.maxwgt) / 2).toFixed(2)
                    }
                }
            }
        }
    },
    {
        ...
        componentProps: ({ formModel }) => {
            return {
                precision: 0,
                onChange: (e) => {
                    console.log(e, formModel.minwgt)
                    if (formModel.minwgt && e) {
                        formModel.avgwgt = ((formModel.minwgt + e) / 2).toFixed(2)
                    }
                }
            }
        }
    ...
    },
    {
        ...
        dynamicRules: () => {
            return [{ required: true, message: '请选择交割时间!' }]
        }
    },
    {
        label: '到',
        field: 'delvedat',
        component: 'DatePicker',
        componentProps: { 'show-time': true, placeholder: '请输入交割结束时间' },
        dynamicRules: () => {
            return [{ required: true, message: '请选择交割时间!' }]
        }
    },
    ...
    },
    {
        ...
        defaultValue:
            '客户须知：该标段价格所对应的销售均重范围为预估出栏均重，实际赶猪均重与中标均重允许误差±5公斤范围，结算单价以实际销售均重为准，下单为自动默认同意实际均重与订单均重误差±5公斤范围。',
        componentProps: {
            placeholder: '请输入说明',
